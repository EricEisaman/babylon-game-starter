...
function createGithubPagesWorkflow(settings) {
  ...
  return `name: Deploy GitHub Pages\n\non:\n  push:\n    branches: [${branchesYaml}]\n  # For workflow_dispatch, use the same branch as on.push (${deployBranch}) so environment rules match.\n  workflow_dispatch:\n\npermissions:\n  contents: read\n  pages: write\n  id-token: write\n\nconcurrency:\n  group: "pages"\n  cancel-in-progress: true\n\njobs:\n  assert_deploy_branch:\n    runs-on: ubuntu-latest\n    steps:\n      - name: Enforce deploy branch for manual runs\n        run: |\n          if [ "\${{ github.event_name }}" = "workflow_dispatch" ] && [ "\${GITHUB_REF}" != "refs/heads/${deployBranch}" ]; then\n            echo "::error::GitHub Pages deploy is only supported from ${deployBranch}. Run workflow with Use workflow from = ${deployBranch}, or push to that branch. (Ref: \${GITHUB_REF})"\n            exit 1\n          fi\n\n  build:\n    runs-on: ubuntu-latest\n    needs: assert_deploy_branch\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: 22\n          cache: npm\n      - run: npm ci\n      - run: npm run build\n      - uses: actions/upload-pages-artifact@v3\n        with:\n          path: dist\n\n  deploy:\n    environment:\n      name: ${environmentYaml}\n      url: \${{ steps.deployment.outputs.page_url }}\n    runs-on: ubuntu-latest\n    needs: build\n    steps:\n      - id: deployment\n        uses: actions/deploy-pages@v4\n`;
}
